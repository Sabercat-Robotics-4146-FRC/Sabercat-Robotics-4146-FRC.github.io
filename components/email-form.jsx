      ...
      if (Email && Email.replace(/\s/g, "").length && validate(Email)) {
        if (Subject && Subject.replace(/\s/g, "").length) {
          if (Message && Message.replace(/\s/g, "").length) {
            ResponseText.innerText = Translate("Attempting");
            ...
            try {
              ...
              if (EmailResponse.VerifiedError) {
                ResponseText.innerText = Translate("Responses.VerifiedError");
                return;
              } else if (EmailResponse.SendError) {
                ResponseText.innerText = Translate("Responses.SendError");
                return;
              };
              ResponseText.innerText = !EmailResponse.Success ? Translate("Responses.ServerError") : Translate("Responses.Success"); 
            } catch(Err) {
              console.warn(Err);
              SetInfoClasses("text-red-500");
              ResponseText.innerText = Translate("Responses.ServerError");    
            };
          } else {
            SetInfoClasses("text-red-500");
            ResponseText.innerText = Translate("Responses.EmptyMessage");  
          };
        } else {
          SetInfoClasses("text-red-500");
          ResponseText.innerText = Translate("Responses.EmptySubject");  
        };
      } else {
        SetInfoClasses("text-red-500");
        ResponseText.innerText = Translate("Responses.InvalidEmail");
      };
    };
  };
  ...
  return (
    <section className="flex flex-col flex-wrap space-y-4 sm:space-y-0 sm:space-x-4 sm:flex-row sm:flex-nowrap justify-center items-center text-center py-16 text-neutral-900/75" id="contact">
      <header className="w-full sm:w-1/2 px-4 mb-4">
        <h2 className="mb-4 font-medium text-3xl text-neutral-900">{Translate("Header")}</h2>
        <p>
          {
            Translate.rich("Description", {
              inlineEmailLink: function(Chunks) {
                return (
                  <a href="mailto:team@sabercatrobotics.com" className="transition-colors duration-300 text-yellow-500 hover:text-yellow-600 focus-visible:text-yellow-600">
                    {Chunks}
                  </a>
                );
              },
      ...
      <form onSubmit={HandleEmailEvent} className="w-full sm:w-1/2 space-y-4">
        <section className="flex flex-col flex-wrap">
          <label className="text-neutral-900 text-start text-2xl font-medium" htmlFor="email">
            <h3>{Translate("Email.Label")}</h3>
          </label>
          <main className="relative flex flex-wrap items-strech w-full border-2 transition-[border-color] duration-300 focus-within:border-yellow-500">
            <input className="order-1 bg-slate-100 flex-1 basis-auto p-3 w-[1%] focus:outline-none transition-[background-color] duration-300 focus:bg-slate-50 peer" inputMode="email" type="email" id="email" placeholder={Translate("Email.Placeholder")} onInput={function(Event) {SetEmail(Event.target.value)}} />
            <div className="p-3 bg-slate-100 fill-neutral-900 transition-[background-color,fill] duration-300 peer-focus:fill-yellow-500 peer-focus:bg-slate-50 flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" height="1em" role="presentation" viewBox="0 0 512 512">
        ...
        <section className="flex flex-col flex-wrap">
          <label className="text-neutral-900 text-start text-2xl font-medium" htmlFor="subject">
            <h3>{Translate("Subject.Label")}</h3>
          </label>
          <main className="relative flex flex-wrap items-strech w-full border-2 transition-[border-color] duration-300 focus-within:border-yellow-500">
            <input className="order-1 bg-slate-100 flex-1 basis-auto p-3 w-[1%] focus:outline-none transition-[background-color] duration-300 focus:bg-slate-50 peer" inputMode="text" type="text" id="subject" placeholder={Translate("Subject.Placeholder")} onInput={function(Event) {SetSubject(Event.target.value)}} />
            <div className="p-3 bg-slate-100 fill-neutral-900 transition-[background-color,fill] duration-300 peer-focus:fill-yellow-500 peer-focus:bg-slate-50 flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" height="1em" role="presentation" viewBox="0 0 512 512">
        ...
        <section className="flex flex-col flex-wrap">
          ...
          <main className="relative flex flex-wrap items-center w-full border-2 transition-[border-color] duration-300 focus-within:border-yellow-500">
            <textarea className="resize-none h-[6ch] order-1 bg-slate-100 flex-1 basis-auto p-3 w-[1%] focus:outline-none transition-[background-color] duration-300 focus:bg-slate-50 peer" inputMode="text" id="message" placeholder={Translate("Message.Placeholder")} onInput={function(Event) {SetMessage(Event.target.value)}} />
        ...
        <section className="relative flex flex-wrap items-strech w-full">
          ...
          <p ref={ResponseRef} className={`w-full ${InfoClasses} mt-2 leading-none`} id="response" />
        </section>
      </form>
    </section>
  );
